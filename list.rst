...
列表的 L.copy() 方法与切片复制都是浅拷贝，只复制父对象一级，子对象不复制，还是引用。
...
Python 内置模块 collections 扩展了普通元组，参考 :ref:`namedtuple` 。 
